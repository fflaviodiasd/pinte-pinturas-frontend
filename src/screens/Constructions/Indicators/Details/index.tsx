...
import { Box, Grid, Tooltip, useTheme, Typography, Checkbox, TextField, List, ListItem, ListItemText, Menu, MenuItem } from "@mui/material";
import { useStyles } from "./styles";
import { useParams, useNavigate } from "react-router-dom";
import { Add, CalendarToday, SquareFoot, Groups, Info } from "@mui/icons-material";
...
const mockTableData = [
  {
    ...
    subRows: [
      { id: 11, service_step: "Etapa 1", total_execution: "12 m²", median: "3 m²/dia", price: "R$ 100,00", total_price: "R$ 100,00" },
      { id: 12, service_step: "Etapa 2", total_execution: "10 m²", median: "3 m²/dia", price: "R$ 100,00", total_price: "R$ 100,00" },
      { id: 13, service_step: "Etapa 3", total_execution: "20 m²", median: "5 m²/dia", price: "R$ 200,00", total_price: "R$ 200,00" },
      { id: 14, service_step: "Etapa 4", total_execution: "6 m²", median: "1 m²/dia", price: "R$ 100,23", total_price: "R$ 100,23" },
    ],
  },
  {
    id: 2,
    ...
    subRows: [
      { id: 21, service_step: "Etapa 1", total_execution: "100L", median: "10L/dia", price: "R$ 3000,00", total_price: "R$ 3000,00" },
      { id: 22, service_step: "Etapa 2", total_execution: "138L", median: "23L/dia", price: "R$ 2250,23", total_price: "R$ 2250,23" },
    ],
  },
];
...
export const Details = () => {
  ...
  const [filteredMeasurements, setFilteredMeasurements] = useState<any[]>(mockTableData);
  const [measurementAnchorEl, setMeasurementAnchorEl] = useState<null | HTMLElement>(null);
  ...
  const table = useMaterialReactTable({
    ...
    renderDetailPanel: ({ row }) => (
      <Box sx={{ padding: '1rem' }}>
        <Typography variant="body2">Detalhes adicionais para {row.original.service_step}</Typography>
      </Box>
    ),
    ...
    <Grid container spacing={2}>
      ...
      >
        <Box sx={{ padding: 2 }}>
          ...
          <MenuItem>
            <Checkbox
              checked={selectedMeasurements.length === mockMeasurements.length}
          ...
          {mockMeasurements.map((measurement) => (
            <MenuItem key={measurement.id}>
              <Checkbox
                ...
                inputProps={{ 'aria-labelledby': `checkbox-list-label-${measurement.id}` }}
              />
              <ListItemText id={`checkbox-list-label-${measurement.id}`} primary={measurement.name} />
            </MenuItem>
          ))}
        </Box>
      </Menu>
          ...
          {mockTeams.map((team) => (
            <MenuItem key={team.id}>
              <Checkbox
                ...
                inputProps={{ 'aria-labelledby': `checkbox-list-label-${team.id}` }}
              />
              <ListItemText id={`checkbox-list-label-${team.id}`} primary={team.name} />
            </MenuItem>
          ))}
        </Box>
      </Menu>
