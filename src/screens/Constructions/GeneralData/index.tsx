  ...
  const columns = useMemo<MRT_ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        aggregationFn: 'sum',
        Footer: ({ table }) => {
          const total = table.getPreFilteredRowModel().rows.reduce((sum, row) => sum + parseFloat(row.original.qnt_step_service || 0), 0);
          return (
            <Box display="flex" flexDirection="column" alignItems="left">
              <Typography fontWeight={400} fontSize={10}>Total</Typography>
              <span>{total}</span>
            </Box>
          );
        },
      },
      {
        ...
        AggregatedCell: ({ cell }: any) => (
          <span>
            Somatória: <Box component="span" sx={{ color: 'green', fontWeight: 'bold' }}>R$ {cell.getValue()}</Box>
          </span>

          
        ),
        aggregationFn: (columnId, leafRows) => leafRows.reduce((sum, row) => sum + parseFloat((row.original.amount_to_receive_step?.toString() || "0").replace(',', '')), 0),
        Footer: ({ table }) => {
          const total = table.getPreFilteredRowModel().rows.reduce((sum, row) => sum + parseFloat((row.original.amount_to_receive_step?.toString() || "0").replace(',', '')), 0);
          return (
            <Box display="flex" flexDirection="column" alignItems="left">
              <Typography fontWeight={400} fontSize={10}>Total</Typography>
              <Box component="span" sx={{ color: 'green' }}>R$ {total.toFixed(2)}</Box>
            </Box>
          );
        },
      },
      {
        ...
        aggregationFn: (columnId, leafRows) => leafRows.reduce((sum, row) => sum + parseFloat((row.original.total_price_service?.toString() || "0").replace(',', '')), 0),
        Footer: ({ table }) => {
          const total = table.getPreFilteredRowModel().rows.reduce((sum, row) => sum + parseFloat((row.original.total_price_service?.toString() || "0").replace(',', '')), 0);
          return (
            <Box display="flex" flexDirection="column" alignItems="left">
              <Typography fontWeight={400} fontSize={10}>Total</Typography>
              <Box component="span" sx={{ color: 'green' }}>R$ {total.toFixed(2)}</Box>
      ...
      },
      {
        ...
        aggregationFn: (columnId, leafRows) => leafRows.reduce((sum, row) => sum + parseFloat((row.original.amount_to_pay?.toString() || "0").replace(',', '')), 0),
        Footer: ({ table }) => {
          const total = table.getPreFilteredRowModel().rows.reduce((sum, row) => sum + parseFloat((row.original.amount_to_pay?.toString() || "0").replace(',', '')), 0);
          return (
            <Box display="flex" flexDirection="column" alignItems="left">
              <Typography fontWeight={400} fontSize={10}>Total</Typography>
              <Box component="span" sx={{ color: 'green' }}>R$ {total.toFixed(2)}</Box>
      ...
      },
      {
        ...
        aggregationFn: (columnId, leafRows) => leafRows.reduce((sum, row) => sum + parseFloat((row.original.total_to_pay?.toString() || "0").replace(',', '')), 0),
        Footer: ({ table }) => {
          const total = table.getPreFilteredRowModel().rows.reduce((sum, row) => sum + parseFloat((row.original.total_to_pay?.toString() || "0").replace(',', '')), 0);
          return (
            <Box display="flex" flexDirection="column" alignItems="left">
              <Typography fontWeight={400} fontSize={10}>Total</Typography>
              <Box component="span" sx={{ color: 'green' }}>R$ {total.toFixed(2)}</Box>
