...
export const useConferenceLocal = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { user } = useContext(UserContext);

  const [loading, setLoading] = useState(true);
  const [listConferenceDataLocal, setListConferenceData] = useState<any[]>([]);
  ...
  const [listReportsWithEmployees, setListReportsWithEmployees] = useState<any[]>([]);
    ...
    const getConferenceDataLocal = async (measurementId: string, disjunction: string) => {
  ...
  return {
    loading,
    setLoading,
    getConferenceDataLocal,
    listConferenceDataLocal,
    fetchMeasurements,
    fetchEmployees,
    fetchTeams,
    getProductionData,
